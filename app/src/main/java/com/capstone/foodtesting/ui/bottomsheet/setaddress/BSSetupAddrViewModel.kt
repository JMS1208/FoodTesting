...
@HiltViewModel
class BSSetupAddrViewModel @Inject constructor(
    ...
    //DataStore
    suspend fun getCurrentAddressInfoUUID() = withContext(Dispatchers.IO) {
        repository.getCurrentAddressInfoUUID().first()
    }


    //Room
    val savedAllAddressInfo: StateFlow<List<AddressInfo>>
        =  repository.getAllAddressInfo().stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), listOf())

    fun insertAddressInfo(addressInfo: AddressInfo) = viewModelScope.launch(Dispatchers.IO) {
        repository.insertAddressInfo(addressInfo)
    }

    fun deleteAddressInfo(addressInfo: AddressInfo) = viewModelScope.launch(Dispatchers.IO) {
        repository.deleteAddressInfo(addressInfo)
    }

    fun deleteAllAddressInfo() = viewModelScope.launch(Dispatchers.IO) {
        repository.deleteAllAddressInfo()
    }
